...
namespace ConsoleApp12
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int x = 5; //закритий ключ
            BigInteger y = BigInteger.Pow(g_main, x) % p; //Відкритий ключ


            Console.Write($"Числа [{p}, {g_main}, {y}] ---> [p, g, y]\n");
            Console.Write($"Вiдкритий ключ - это <y>\n");
            Console.Write($"Закритий ключ: {x}\n");


            Console.Write($" Зчитування файлу: ");
            string text = System.IO.File.ReadAllText(@"C:\Labs\Kripto\2\2\elga\elga\bin\Debug\net6.0\1.txt");
       


            List<BigInteger> array_cipher_text = new List<BigInteger>();
            array_cipher_text = Cipher(text, p, y);
            Console.WriteLine("Зашифроване повiдомлення: ");
            for (int i = 0; i != text.Length; i++)
            {
            ...
            Console.Write($"\nДешифроване повiдомлення: {text = Cipher_RAZ(text.Length, array_cipher_text, x, p)}");
